...
multi_tuv <- function(df, site = "name", pah, varying, vals = NULL, ...) {
  ...
  df |> 
    ...
    mutate(
      ...
      plc_nlc_ratio = plc50 / nlc50_lookup(pah)
    )
}
...
We then use the `multi_tuv()` function to calculate $P_{abs}$, PLC50, and ratio of PLC50:NLC50 for Anthracene using recorded DOC values at the three sites:
...
$k_{d,305} = a_{305}[DOC]^{b_{305}} + 0.13$; $a_{305}$ = 2.76 and $b_{305}$ = 1.23 (Morris et al 1995; Equation 4-1a in ARIS 2023a)
...
Using sample $k_d$ values from Table 3 (ARIS 2023a), from 0.08 to 275:
...
Therefore DOC ~= 40 mg/L when $k_d(305)$ is ~250. This is a significantly higher DOC 
concentration than observed in our three sample lakes, and is in fact outside the
range where the above equation is recommended (it is recommended for [DOC] between 0.2 and 23 mg/L).
...
Using sample surface albedo values from 0.05 to 0.1 (as suggested in ARIS 2023b), 
and include the suggested default of 0.7:
...
the `o3_tc` argument in the `set_tuv_aq_params` function.

To test the sensitivity of PLC50 to ozone column, a range of values is tested based on 
the recommendation in ARIS 2023b. The recommended range is 280-420 DU.
...
The pahwq package currently uses average monthly aerosol optical depth data from 2002 to 2023, obtained from MODIS/Aqua satellite data. Based on latitude and longitude and month, the AOD value is looked up and supplied to the TUV model. This default behaviour can be overridden by supplying a value to the `tauaer` argument in the `set_tuv_aq_params` function.

To test the sensitivity of PLC50 to AOD, a range of values is tested based on 
the recommendation in ARIS 2023b. The recommended range is 0.1 to 1.0.

<details>
<summary>Show/Hide Code</summary>
```{r}
aod_vals <- c(seq(0.1, 1.0, length.out = 10))
```
</details>
...
As recommended in ARIS 2023b, here we test a range for values from 0.80 to 0.99.

<details>
<summary>Show/Hide Code</summary>
```{r}
ssa_vals <- c(seq(0.8, 0.99, length.out = 10))
```
</details>
