...
ggplot(out, aes(x = name, y = plc50, colour = nstr)) + 
  geom_point() + 
  labs(title = "Calculated PLC50 using two different TUV methods for radiative transfer", x = "Site") + 
  theme(axis.text.x = element_text(angle = 355))

ggplot(out, aes(x = name, y = timing, colour = nstr)) + 
  geom_point() + 
  labs(title = "Timing of running TUV model with two different methods for radiative transfer", x = "Site",
       y = "Execution Time (s)") + 
  theme(axis.text.x = element_text(angle = 355))
```
...
pahwq:::molar_absorption |> 
  ...
  geom_point() + 
  labs(y = "Molar absorption (L/mol/cm)")
```
