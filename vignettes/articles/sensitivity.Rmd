...
<!-- to render this as a pdf, run:
quarto render vignettes/articles/sensitivity.Rmd -o "sensitivity_2024-10-28.pdf" --to typst
-->
...
#| echo: false
diff_DOC |>
  select(-doc_min, -doc_max, -tuv_res, -timing, -date, -emsid) |>
  gt() |>
  cols_label_with(fn = \(x) gsub("_", " ", x)) |>
...
#| echo: false
same_DOC |>
  select(-doc_min, -doc_max, -tuv_res, -emsid, -date, -timing, -DOC) |>
  gt() |>
  cols_label_with(fn = \(x) gsub("_", " ", x)) |>
  data_color(columns = phototoxic_benchmark, palette = "Greens") |>
...
$a_{305}$ = 1.28 and $b_{305}$ = 1.31 from @morris1995; Equation 6 in @aris2024
...
is ~56. This is a significantly higher DOC concentration than observed in our
three sample lakes, and is near the maximum of the range for which the above equation is
recommended (it is recommended for [DOC] between 0.2 and 61.45 mg/L).
