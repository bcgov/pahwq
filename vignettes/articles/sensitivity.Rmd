...
We will first define a funtion, `multi_tuv()` that allows us to do repeated 
...
For Anthracene, the ratio of PLC50:NLC50 approaches 1 when $k_d(305)$ is ~ 250 --- in other words, 
PLC50 is nearly the same as NLC50, indicating that the light is attenuated to such 
an extent that the phototoxicity of Anthracene is not activated at the light levels when $k_d(305)$ >= 250.
...
* Recommended test range 280-420 DU

```{r}
ozone_vals <- seq(280, 420, by = 10)
```

### Ozone Total Column for Anthracene
...
### Ozone Total Column for Benzo[a]pyrene
...
### Aerosol Optical Depth (AOD) for Anthracene
...
### Aerosol Optical Depth (AOD) for Benzo[a]pyrene
...
### Aerosol Single Scattering Albedo for Anthracene
...
### Aerosol Single Scattering Albedo for Benzo[a]pyrene
