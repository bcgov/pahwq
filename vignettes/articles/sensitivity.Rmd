...
(nlc50_plot <- pahwq:::nlc50_lookup |> 
   ...
   ggplot(aes(
     x = log_kow, y = nlc50, colour = chemical
   )) + 
   geom_point())
```
...
We can also compare the ratio of PLC50:NLC50:

```{r, hide_src=TRUE}
ggplot(naphthalene, aes(x = PAH, y = plc_nlc_ratio, colour = PAH)) + 
  geom_point() + 
  facet_wrap(vars(Kd_ref), ncol = 1, scales = "free_y", 
             labeller = as_labeller(function(x) paste0("Kd(305) = ", x)))
```
