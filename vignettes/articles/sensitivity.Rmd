...
To perform the sensitivity analysis, we need to define a function, `multi_tuv()` that allows us to do repeated runs of the TUV model and PLC50 calculation using a range of inputs at multiple sites:
...
We then use the `multi_tuv()` function to calculate $P_{abs}$, PLC50, and ratio 
...
ggplot(lat_test_b, aes(x = lat, y = plc_nlc_ratio)) +
  ...
  theme_bw() + 
  labs(
    title = "Ratio of PLC50:NLC50 for Benzo[a]pyrene at 3 sites in B.C. for a range of latitudes",
    x = "Latitude",
    y = "Ratio of PLC50:NLC50"
  )
```
...
ggplot(elev_test_a, aes(x = elev_km, y = plc_nlc_ratio)) +
  ...
  theme_bw() + 
  labs(
    title = "Ratio of PLC50:NLC50 for Anthracene at 3 sites in B.C. for a range of elevations",
    x = "Elevation (km)",
    y = "Ratio of PLC50:NLC50"
  )
```
...
ggplot(elev_test_b, aes(x = elev_km, y = plc_nlc_ratio)) +
  ...
  theme_bw() + 
  labs(
    title = "Ratio of PLC50:NLC50 for Benzo[a]pyrene at 3 sites in B.C. for a range of elevations",
    x = "Elevation (km)",
    y = "Ratio of PLC50:NLC50"
  )
```
...
ggplot(depth_test_a, aes(x = depth_m, y = plc_nlc_ratio)) +
  ...
  theme_bw() + 
  labs(
    title = "Ratio of PLC50:NLC50 for Anthracene at 3 sites in B.C. for a range of water depths",
    x = "Water Depth (m)",
    y = "Ratio of PLC50:NLC50"
  )
```
...
ggplot(depth_test_b, aes(x = depth_m, y = plc_nlc_ratio)) +
  ...
  theme_bw() + 
  labs(
    title = "Ratio of PLC50:NLC50 for Benzo[a]pyrene at 3 sites in B.C. for a range of water depths",
    x = "Water Depth (m)",
    y = "Ratio of PLC50:NLC50"
  )
```
